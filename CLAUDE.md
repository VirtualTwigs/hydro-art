...
Self-contained HTML/JS mockups (no build, no server — open directly over `file://`) exploring a future interactive UX for the generator; **not wired to the Python pipeline**, and `src/` + the offline test suite must stay free of any dependency on `web/`. `index.html` is an early standalone control-panel concept; `3d.html` is a WebGL "3D Terrain Lab" that orbits the terrain-and-rivers model — it consumes the DEM-backed heightfield tiles emitted by `src/preview.py` (replacing its earlier synthetic `elevationAt()` field).
...
**Prototypes → product.** `proto-a-studio.html` (dense left-rail studio), `proto-b-guided.html` (step-by-step wizard), and `proto-c-canvas.html` (canvas-first toolbar/popovers) are three UX explorations over the shared foundation. The **chosen direction** (roadmap #26, `agent-os/specs/2026-08-10-web-control-surface/`) is Prototype A's studio layout using Prototype B's click-to-set month timeline. That control surface is a *mapping target only* — it drives a live client-side preview and emits a deterministic CLI/YAML output contract from one mutable `state` object, but does not run the pipeline (#27). Options that map to not-yet-shipped roadmap items #23–#25 (`color_by` single/elevation, `width_by` flow min/max/gamma, `--county`, `--months`) render with an explicit `(proposed)` marker until those land.
