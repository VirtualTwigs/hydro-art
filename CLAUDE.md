...
**This 12-stage pipeline is 2D-only.** The elevation/DEM/terrain/3D, flow, report, and fulfillment subsystems are **parallel** pure/offline modules **not** wired into `PIPELINE_STAGES` and not run by `build.py` — a second data model sharing CRS conventions with its own `tools/` entry points. Exceptions that *do* integrate: waterbodies (Epoch 1.5) and natural water features (Epoch 15) — `validate` additively loads the `NHDWaterbody` polygon layer when `settings.waterbodies.enabled` **or** `settings.areal_features.enabled` (complementary taxonomies over one load), and separately loads `NHDPoint` when `settings.point_features.enabled`; `generate_svg` selects all of these (outline layers, areal fills, point glyphs). All additive loads are gated on both the setting AND loader support, so river-only/disabled builds stay byte-identical.
