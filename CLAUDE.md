...
**Natural water features (Epoch 15, *integrated* into pipeline):** classification mirrors `waterbodies` (FType/FCode policy tables, name kept for provenance only, no geometry math, no GIS imports — runs offline). `point_features` (`NHDPoint` → springs/waterfalls/rapids `PointFeature`s) · `areal_features` (`NHDWaterbody` → wetlands/playas/perennial-ice `ArealFeature`s — a **complementary** taxonomy to `waterbodies`, so each polygon is classified by exactly one taxonomy) · `areal_selection` (repair→reproject→clip for both, then per-class `min_area_m2` for polygons and per-family `min_spacing_m` density-thinning for points; `process_areal_features`/`process_point_features`). Config via `PointFeatureSettings`/`ArealFeatureSettings` (+ `POINT_FEATURE_PRESETS`), both disabled by default.

**Hydro structures (Epoch 16, *integrated* into pipeline):** engineered-water infrastructure — dams/weirs, gates, lock chambers, spillways, gaging stations, water intakes/outflows, canals/ditches. `hydro_structures` (`classify_hydro_structure_layer` → `HydroStructure`; one FType/FCode policy table spans all THREE source layers `NHDLine`/`NHDPoint`/`NHDArea` since a code means the same on each — a **complementary**, disjoint taxonomy to waterbodies/point/areal features; `436 Reservoir` intentionally stays with `waterbodies`; no geometry math, no GIS imports — runs offline) · `hydro_structure_selection` (`process_hydro_structures` repair→reproject→clip + policy) · `hydro_structure_qa` (Epoch 16 #68, pure/offline QA over classified structures + flowlines: on-network placement, cross-layer duplicate suppression, canal/natural separation; top-level `import shapely` only, projection-free — caller reprojects to EPSG:5070 first). `generate_svg` draws structures above the water layers via `render_svg`'s `hydro_structures` param + `DEFAULT_HYDRO_STRUCTURE_STYLES` (per-class), bridged from group-level `HydroStructureSettings` (+ `HYDRO_STRUCTURE_PRESETS` `screen`/`print-county`/`print-state`, monotonic thinning). Disabled by default → byte-identical default build.
...
- **Generation 1 — production endpoints, gallery & release (Epochs 19–23):** all pure/stdlib, byte-stable (`sort_keys`), none in `PIPELINE_STAGES`; real renders live in matching non-offline `tools/`. `endpoints` (Epoch 19 — generalizes `fulfillment` from one print order to the **four production endpoints** `digital_image`/`animation`/`print_image`/`report`, each with a versioned output *contract*, fail-fast request validator, deterministic plan, provenance manifest, and injectable `dispatch_endpoint` seam; also `flagship_e2e_requests`/`e2e_contract_digest` for the all-four proof, Epoch 21) · `gallery` (Epoch 22 — curated rights-clean style matrix `GALLERY_MATRIX` + deterministic `gallery_ledger` provenance/checksum/sellable ledger) · `release` (Epoch 23 — **pure half** of the v1.0 release gate: recomputes render-independent goldens + aggregates `DeterminismVerdict`s into a ready/blocked `ReleaseVerdict`) · `changelog` (Epoch 23 — parses roadmap Markdown → deterministic `CHANGELOG.md`, mirroring `status`/`unfinished`).
- **Determinism & bookkeeping (Epoch 9):** `determinism` (golden-hash registry `tests/fixtures/golden/registry.json` + verdict; double-render is `tools/verify_determinism.py`) · `status` (idempotent `HANDOFF.md`/roadmap string transforms; CLI is `tools/update_status.py`) · `unfinished` (pure string/data transforms over spec/HANDOFF/retrospective *contents* → punch list of loose ends; report-only, the fs/git walk + printing live in `tools/detect_unfinished.py`, wired to a SessionStart hook).
...
- *Determinism/status (Epoch 9):* `verify_determinism` (double-render + golden), `update_status` (epoch-close bookkeeping), `detect_unfinished` (report-only punch list of unfinished work; exit 1 when anything's found — the SessionStart hook's entry point).
- *Stylized 3D:* `render_county_3d`/`render_state_3d` (height from Strahler order — stylized, NOT surveyed DEM), `waterbody_qa`.
- *Hydro-structure QA (Epoch 16):* `hydro_structure_qa` (non-offline; loads real `NHDLine`/`NHDPoint`/`NHDArea`, classifies+selects via the pipeline code, runs `src.hydro_structure_qa.build_qa_report` against the real flowline network, prints placement/duplicates/separation — mirrors `waterbody_qa.py`). `render_state_allfeatures.py` gains an opt-in `--structures` flag to draw them.
...
- *Production endpoints / gallery / release (Gen 1, Epochs 19–23):* `render_endpoint`/`render_all_endpoints` (non-offline renderers injected into `src.endpoints.dispatch_endpoint`; the flagship all-four proof), `render_gallery` (real hi-res bytes for the `src.gallery` matrix), `release_gate` (real double-render + `src.release` verdict), `build_changelog` (roadmap → `CHANGELOG.md`), `coverage_report` (offline coverage gate, `--fail-under`).
- *Museum print concepts:* `museum_common` (shared finishing: transparent hi-res raster + Fraunces/DM-Mono vector layout → proof PNG via `resvg` + print PDF via `rsvg-convert`, OFL fonts from `assets/fonts`) + `render_museum_{almanac,compass,divide,index,specimen}` (five framed print directions from `experiments/museum-print-layouts.html`).

### Web prototypes (`web/`)

Self-contained HTML/JS (no build step, `file://`-safe). The **official surface** is the alpha customer journey (`web/start.html` → the four catalog pages) plus the expert console. `studio.html` is the **canonical control surface** (roadmap #26) — when *served* by `serve.py` its "Run pipeline" button POSTs `HydroUX.renderRequest(state)` (structured JSON of real `src/config` keys) to the localhost runner; over `file://` it stays mapping-only. The other official pages: `start.html` (alpha landing, Epoch 24 e2e-tested), `proto-b-guided.html` (poster catalog page), `proto-c-canvas.html` (animation catalog page), `report.html` (watershed-report viewer over `src/flow_metrics`), `gallery.html` (marketing gallery viewer over the `src/gallery` rights ledger). **Archived (2026-09-12, not part of the official product):** `index.html` (early concept), `3d.html` (WebGL terrain lab over `src/preview.py`), `experience.html` (hillshade + camera viewer over `src/delivery.py`) now live under `archive/web/` — the `src/preview.py`/`src/delivery.py` subsystems remain, but their prototype UIs are archived.
