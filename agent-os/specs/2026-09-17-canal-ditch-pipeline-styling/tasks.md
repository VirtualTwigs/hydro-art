...
- [x] 1.1 Create `src/flowline_channels.py` with `FLOWLINE_CHANNEL_POLICY_VERSION`,
...
- [x] 1.2 Implement `classify_ftype(ftype)` → channel class string. Missing/unknown → `"stream"`.
- [x] 1.3 Implement `build_channel_dashes(edge_ftypes, dashes=None)` → `dict[int, str]`.
...
- [x] 1.4 Create `tests/test_flowline_channels.py`:
...
- [x] 2.1 Add `FLOWLINE_ATTRIBUTE_FIELDS = ("FType", "FCode")` to `src/loading.py`.
- [x] 2.2 Add `include_attributes: bool = False` parameter to `PyogrioLayerLoader.load_layers()`.
...
- [x] 2.3 Modify `build_graph()` in `src/graph.py`: when a `Layer` has `attributes`, store
...
- [x] 2.4 Tests in `tests/test_loading.py` and `tests/test_graph.py`:
...
- [x] 3.1 Add `channel_dashes: Mapping[int, str] | None = None` parameter to `_path_element`
  in `src/rendering.py`. When present and segment_id is in the mapping, emit
  `stroke-dasharray="<value>"` on the `<path>`.
- [x] 3.2 Thread `channel_dashes` through `_group_lines`, `_halo_lines` → `_path_element`.
- [x] 3.3 Thread `channel_dashes` through `_render_lines`, `render_svg`, `render_svg_stream`.
- [x] 3.4 Tests in `tests/test_rendering_svg.py`:
...
- [x] 4.1 Add `FlowlineChannelSettings` frozen dataclass to `src/config.py`:
  `enabled: bool`, `dashes: dict[str, str]`.
- [x] 4.2 Add `DEFAULTS["flowline_channels"]` and `FLOWLINE_CHANNEL_PRESETS` dict
  (`screen`, `print-state`, `print-county`).
- [x] 4.3 Add `_coerce_flowline_channels()` validation function following the existing
  `_coerce_hydro_structures` pattern. Wire into `build_settings`.
- [x] 4.4 Add `--flowline-channels` (enable flag) and `--flowline-channel-preset` to
  `src/cli.py`.
- [x] 4.5 Tests in `tests/test_config.py` and `tests/test_build.py`:
...
- [x] 5.1 In `_validate_stage` (`src/pipeline.py`): when `settings.flowline_channels.enabled`,
  pass `include_attributes=True` to the loader.
- [x] 5.2 In `_generate_svg_stage`: when `settings.flowline_channels.enabled`, iterate graph
...
- [x] 5.3 Tests in `tests/test_pipeline.py` and `tests/test_rendering_pipeline.py`:
...
- [x] 6.1 Run full suite: `.venv/bin/python -m pytest -q` — all green (1133 passed), no regressions.
- [x] 6.2 Run recipe roundtrip: `node tests/test_recipe_roundtrip.cjs` — green (11/11).
- [ ] 6.3 Verify byte-identical default: compare SVG sha256 of a default build (no
  ...
  *Deferred — requires real GIS data render (non-offline).*
- [x] 6.4 Write `implementation/report.md`.
