...
- [x] Write tests first (`tests/test_storage.py`): with no external root and
...
- [x] `src/storage.py`: `StorageError`, `DEFAULT_LOCAL_ROOTS`,
...
- [x] Run ONLY the new tests; green (8 passed).

## TG2 — Migration planner + executor (`src/storage.py`)

- [x] Write tests first (`tests/test_storage.py`): `plan_migration` over a
...
- [x] `src/storage.py`: frozen `MigrationItem` / `MigrationPlan` /
...
- [x] Run ONLY the new tests; green (14 passed).

## TG3 — Entry-point wiring (`build.py`, `serve.py`)

- [x] Write/extend tests (`tests/test_build.py`, `tests/test_serve.py`): `build.py`
...
- [x] `build.py`: add the flags, call `resolve_storage`, pass
...
- [x] Run ONLY the new/changed tests; green (26 passed).

## TG4 — Migration CLI + optional local staging + verify/docs

- [x] Add `tools/migrate_storage.py`: a thin CLI over `plan_migration` /
      ...
      guard). (`tests/test_migrate_storage.py`, 4 passed — stdlib-only so it runs
      in the offline suite.)
- [x] (Optional local working copy) Thread `StorageRoots.staging` into the export
      ...
      cover the move-on-finish path. (build.py forwards `roots.staging`.)
- [x] Confirm `src/storage.py` imports offline (no numpy/GDAL pulled in); `ruff`
      if available. (Offline import verified; ruff not installed in `.venv`.)
- [x] Run the full Python suite (regression check); note the count. (488 passed,
      was 462; +26.)
- [x] Write `implementation/report.md`; tick this `tasks.md`; update `HANDOFF.md`,
