...
- **Group 3 — #50** `src/flow_validation.py`: `bias`/`pearson_r`/`nash_sutcliffe`/`rmse`,
  ...
  location, snapshotted). `tests/test_flow_validation.py`.
- **Group 4 — #51** `align_index`/`correlate` (+ lag) in `src/flow_validation.py`;
...
- **2D default output byte-identical:** Epoch 12 added only pure `src/flow_metrics.py`
  + `src/flow_validation.py`, neither in `PIPELINE_STAGES`; no pipeline stage touched.
...
- Nothing enters `PIPELINE_STAGES`; `src/flow_metrics.py`/`src/flow_validation.py`
  import numpy only; the offline suite stays GDAL/network-free.
